...
To update the database use this command from the Web project folder (replace `PipelineProcess.api` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../PipelineProcess.api.Infrastructure/PipelineProcess.api.Infrastructure.csproj -s PipelineProcess.api.Web.csproj
```
